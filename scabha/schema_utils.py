...
def schema_to_dataclass(io: Dict[str, Parameter], class_name: str, bases=(), post_init: Optional[Callable] = None):
    ...
    for name, schema in io.items():
        ...
        if required and schema.default is not UNSET:
            raise SchemaError(
                f"Field '{fldname}' is required but specifies a default. This behaviour is unsupported/ambiguous."
            )

        if required:
            fld = field(default=MISSING, metadata=metadata)
        elif isinstance(schema.default, MutableSequence):
            fld = field(default_factory=default_wrapper(list, schema.default), metadata=metadata)
        elif isinstance(schema.default, MutableSet):
            fld = field(default_factory=default_wrapper(set, schema.default), metadata=metadata)
        elif isinstance(schema.default, MutableMapping):
            fld = field(default_factory=default_wrapper(dict, schema.default), metadata=metadata)
...
def nested_schema_to_dataclass(nested: Dict[str, Dict], class_name: str, bases=(), section_bases=(), post_init_map={}):
    ...
    nested_structure = make_dataclass(
        f"_{class_name}_schemas", [(name, Dict[str, Parameter]) for name in nested.keys()]
    )
    ...
    for section, content in nested.items():
        dcls = schema_to_dataclass(
            content, f"{class_name}_{section}", bases=section_bases, post_init=post_init_map.get(section)
        )
...
def clickify_parameters(schemas: Union[str, Dict[str, Any]], default_policies: Dict[str, Any] = None):
    ...
    if type(schemas) is str:
        schemas = OmegaConf.merge(OmegaConf.structured(Schema), OmegaConf.load(schemas))
...
def paramfile_loader(
    paramfiles: Union[File, List[File]], sources: Union[File, List[File]] = [], schema_spec=None, use_cache=False
) -> Dict:
    ...
    struct_args, _ = configuratt.load_nested(paramfiles, structured=args_defn, use_sources=srcs, use_cache=use_cache)
