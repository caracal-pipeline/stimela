...
class PodProxy(object):
        ...
        self.pod_spec = dict(
            containers=[
                dict(
                    ...
                    volumeMounts=[dict(name="home-directory", mountPath=self.uinfo.home)],
                )
            ],
            volumes=[dict(name="home-directory", emptyDir=dict(medium="Memory") if self.uinfo.home_ramdisk else {})],
    ...
    def session_init_container(self):
        if self._session_init_container is None:
            self._session_init_container = dict(
                name="volume-session-init", image="quay.io/quay/busybox", command=["/bin/sh", "-c", ""], volumeMounts=[]
            )
    ...
    def step_init_container(self):
        if self._step_init_container is None:
            self._step_init_container = dict(
                ...
                command=["/bin/sh", "-c", ""],
                securityContext=dict(
                    runAsNonRoot=self.uinfo.uid != 0, runAsUser=self.uinfo.uid, runAsGroup=self.uinfo.gid
                ),
                volumeMounts=[],
            )
    ...
    def add_init_container_mount(cont, volume_name, mount):
        for mnt in cont["volumeMounts"]:
            if mnt["mountPath"] == mount:
                return
        cont["volumeMounts"].append(dict(name=volume_name, mountPath=mount))
    ...
    def add_volume(self, volume_name, mount):
        ...
        self.pod_spec["containers"][0]["volumeMounts"].append(dict(name=volume_name, mountPath=mount))
    ...
    def dispatch_container_logs(self, style: str, job: bool = True):
        ...
        for cont in containers:
            contname = cont["name"]
            try:
                loglines = self.kube_api.read_namespaced_pod_log(
                    name=self.name, namespace=self.namespace, container=contname
                )
    ...
    def _print_logs(self, name, style, container=None):
        ...
        dispatch_to_log(
            self.log,
            f"started logging thread for {name} {container}",
            name,
            "stdout",
            prefix=prefix,
            style=style,
        ...
        try:
            while not self._exit_logging_threads:
                try:
                    ...
                    stream = self.kube_api.read_namespaced_pod_log(
                        name=name, namespace=self.namespace, container=container, _preload_content=False, follow=True
                    )
    ...
    def add_file_check_commands(self, params: Dict[str, Parameter], cab: Cab):
        ...
        must_exist_list, mkdir_list, remove_if_exists_list, active_mounts = resolve_remote_mounts(
            params, cab.inputs, cab.outputs, cwd=self.kube.dir, mounts=set(self._mounts.keys())
        )
        ...
        if must_exist_list or mkdir_list or remove_if_exists_list:
            ...
            for path in remove_if_exists_list:
                self.add_init_container_command(
                    cont,
                    f"ls {path} >/dev/null; if test -e {path}; then echo Removing {path}; rm -fr {path}; true; fi; ",
                )
    ...
    def start_logging_thread(self, name, style, container=None):
        ...
        thread = threading.Thread(target=self._print_logs, kwargs=dict(name=name, container=container, style=style))
    ...
    def run_pod_command(self, command, cmdname, input=None, wrangler=None):
        ...
        resp = stream(
            self.kube_api.connect_get_namespaced_pod_exec,
            self.name,
            self.namespace,
            command=command,
            stderr=True,
