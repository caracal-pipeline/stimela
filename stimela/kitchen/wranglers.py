...
class DeclareSuccess(_BaseWrangler):
    ...
    def apply(self, cabstat: CabStatus, output: str, match: re.Match):
        ...
        return FunkyMessage(
            f"[bold green]{output}[/bold green]", output, prefix=FunkyMessage(":white_check_mark:", "+")
        ), None
...
class ParseOutput(_BaseWrangler):
        ...
        if group in regex.groupindex:
            ...
            if gid > regex.groups:
                raise CabValidationError(
                    f"wrangler action '{spec}' for '{regex.pattern}': {gid} is not a valid ()-group"
                )
            self.gid = gid
        else:
    ...
    def apply(self, cabstat: CabStatus, output: str, match: re.Match):
        ...
        try:
            ...
            cabstat.declare_failure(
                StimelaCabOutputError(f"error parsing string \"{value}\" for output '{self.name}'", exc)
            )
        ...
        for name, value in match.groupdict().items():
            if value is not None:
                try:
                    ...
                    cabstat.declare_failure(
                        StimelaCabOutputError(f"error parsing string \"{value}\" for output '{name}'", exc)
                    )
